...
namespace NntpClientLib
{
    internal class NntpStreamReader : TextReader
    {
        ...
        internal void Initialize(Stream stream, Encoding encoding, int bufferSize)
        {
            ...
            if (!stream.CanRead)
            {
                throw new ArgumentException(NntpErrorMessages.ERROR_44);
            }
            if (bufferSize <= 0)
            {
                throw new ArgumentException(NntpErrorMessages.ERROR_43, "bufferSize");
            }
        ...
        private void CheckObjectState()
        {
            ...
            {
                throw new InvalidOperationException(NntpErrorMessages.ERROR_45);
            }
        }
    }
}
